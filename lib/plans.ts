...
export const PLAN_CONFIG: Record<Plan, PlanDefinition> = {
  ...
  BUSINESS: { id:"BUSINESS", name:"Business", monthlyCents:3999, yearlyCents:39900, maxAssets:1000, maxStorageBytes:50*GB, maxSeats:10, maxSharedWorkspaces:null, description:"Für Teams und kleinere Unternehmen mit Betriebsmitteln und Wartungen." },
};

export function normalizePlan(value: unknown): Plan { return value === "PLUS" || value === "FAMILY" || value === "BUSINESS" ? value : "FREE"; }
export function getPlanDefinition(plan: unknown) { return PLAN_CONFIG[normalizePlan(plan)]; }
export function formatStorage(bytes: number) { if (bytes >= GB) return `${(bytes / GB).toLocaleString("de-DE", { maximumFractionDigits: 1 })} GB`; return `${Math.max(0, Math.round(bytes / MB)).toLocaleString("de-DE")} MB`; }
export function formatEuro(cents: number) { return new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(cents / 100); }
