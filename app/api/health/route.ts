...
export async function GET() {
  try {
    ...
    const ok = Boolean(schema?.workspaces && schema.workspace_members && schema.workspace_invites && schema.asset_workspace_column && schema.reminder_column);
    return Response.json({ ok, service: "navopass", version: "0.4.1", database: true, schema }, { status: ok ? 200 : 503 });
  } catch (error) {
    console.error("NavoPass health check failed", error);
    return Response.json({ ok: false, service: "navopass", version: "0.4.1", database: false, schema: null }, { status: 503 });
  }
}
