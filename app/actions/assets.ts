...
function revalidateAsset(assetId:string,publicId?:string){revalidatePath("/app");revalidatePath("/app/service");revalidatePath("/app/activity");revalidatePath("/app/notifications");revalidatePath("/app/team");revalidatePath(`/app/assets/${assetId}`);if(publicId)revalidatePath(`/p/${publicId}`);}
...
export async function updateAssetAction(formData:FormData){const user=await requireUser();const assetId=text(formData,"assetId",80);const name=text(formData,"name",160);if(!assetId||!name)return;const asset=await getOwnedAsset(user.id,assetId);if(!asset||!roleCanEdit(asset,user.id))return;let workspaceId=asset.workspace_id;let ownerId=asset.owner_id;const requestedWorkspace=text(formData,"workspaceId",80);if(requestedWorkspace&&requestedWorkspace!==asset.workspace_id&&roleCanTransfer(asset,user.id)){const membership=await getWorkspaceMembership(user.id,requestedWorkspace);if(membership&&canEdit(membership.role)){workspaceId=membership.id;ownerId=membership.owner_id;}}await query(`UPDATE assets SET name=$1,category=$2,manufacturer=$3,model=$4,serial_number=$5,purchase_date=$6,warranty_until=$7,next_service_date=$8,service_interval_months=$9,location=$10,notes=$11,visibility=$12,owner_id=$13,workspace_id=$14,updated_at=now() WHERE id=$15`,[name,text(formData,"category",80)??"Sonstiges",text(formData,"manufacturer",160),text(formData,"model",160),text(formData,"serialNumber",160),text(formData,"purchaseDate",20),text(formData,"warrantyUntil",20),text(formData,"nextServiceDate",20),integer(formData,"serviceIntervalMonths",asset.service_interval_months||12,1,120),text(formData,"location",200),text(formData,"notes",5000),visibilityValue(formData),ownerId,workspaceId,assetId]);revalidateAsset(assetId,asset.public_id);redirect(`/app/assets/${assetId}`);}
