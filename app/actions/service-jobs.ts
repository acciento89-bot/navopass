...
export async function createServiceJobAction(formData: FormData) {
  ...
  await query(
    `INSERT INTO service_jobs (user_id,customer_id,asset_id,title,scheduled_for,notes)
     VALUES ($1,$2,$3,$4,$5::timestamptz,$6)`,
    [user.id, customerId || null, asset.id, title, scheduledFor, notes]
  );
...
export async function sendServiceReportAction(formData: FormData) {
  ...
  const event = (await query<{ id: string; title: string; event_date: string }>(
    "SELECT id,title,event_date FROM asset_events WHERE id=$1 AND asset_id=$2 AND event_type IN ('SERVICE','REPAIR','INSPECTION') LIMIT 1",
    [eventId, asset.id]
  )).rows[0];
  ...
  await query(
    `INSERT INTO service_report_shares (asset_id,event_id,created_by,recipient_email,token_hash,expires_at)
