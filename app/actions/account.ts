...
import { createSession, destroySession, findUserByEmail, hashPassword, normalizeEmail, requireUser, verifyPassword } from "@/lib/auth";
...
export async function changePasswordAction(formData: FormData) {
  ...
  if (!row || !(await verifyPassword(currentPassword, row.password_hash))) {
    redirect("/app/settings?passwordError=Aktuelles%20Passwort%20ist%20falsch");
  }
...
export async function deleteAccountAction(formData: FormData) {
  ...
  if (confirmation !== "LÖSCHEN" && confirmation !== "LOESCHEN") {
    redirect("/app/settings?deleteError=Bitte%20LOESCHEN%20eingeben");
  }
  if (!row || !(await verifyPassword(password, row.password_hash))) {
    redirect("/app/settings?deleteError=Passwort%20ist%20falsch");
  }
  ...
  await Promise.all(docs.rows.map(async (doc) => {
