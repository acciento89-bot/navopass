  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.shell}>
        <aside className={styles.side}><div className={styles.sideInner}><span className={styles.eyebrow}>In wenigen Sekunden startklar</span><h2>Dein erster digitaler Pass wartet.</h2><p>Lege dein Konto an und verwalte Geräte, Fahrzeuge, Werkzeuge und vieles mehr an einem Ort.</p><div className={styles.benefits}><div className={styles.benefit}><span className={styles.check}>✓</span><span>Kostenlos starten und den ersten Pass direkt anlegen</span></div><div className={styles.benefit}><span className={styles.check}>✓</span><span>Dokumente, Wartungen und Garantien übersichtlich sammeln</span></div><div className={styles.benefit}><span className={styles.check}>✓</span><span>Auf Wunsch mit Haushalt oder Team zusammenarbeiten</span></div></div></div></aside>
        <div className={styles.formPane}><div className={styles.formBox}><span>Dein NavoPass</span><h1>Konto erstellen</h1><p className={styles.intro}>Einmal registrieren und danach deine digitalen Objektpässe zentral verwalten.</p>{error && <p className={styles.error}>{error}</p>}<form action={registerAction} className={styles.form}><input type="hidden" name="next" value={next} /><label className={styles.label}>Name<input name="name" autoComplete="name" placeholder="Dein Name" required /></label><label className={styles.label}>E-Mail-Adresse<input name="email" type="email" autoComplete="email" placeholder="name@beispiel.de" required /></label><label className={styles.label}>Passwort<input name="password" type="password" minLength={8} autoComplete="new-password" placeholder="Mindestens 8 Zeichen" required /><small className={styles.hint}>Mindestens 8 Zeichen.</small></label><button className={styles.submit} type="submit">Kostenlos registrieren →</button></form><p className={styles.foot}>Schon registriert? <Link href={loginHref}>Jetzt anmelden</Link></p><div className={styles.trust}><span>✓ Datenschutz im Fokus</span><span>✓ Jederzeit verfügbar</span></div></div></div>
      </section>
    </main>
  );
}
