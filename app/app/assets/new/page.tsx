...
export default async function NewAssetPage({ searchParams }: { searchParams: Promise<{ error?: string; upgrade?: string }> }) {
  ...
  return (
    <main className="app-page"><div className="container"><AppHeader name={user.name} /><div className="page-back"><Link href="/app">← Meine Pässe</Link></div><section className="editor-card"><div className="section-heading left"><span className="eyebrow">Neuer Objektpass</span><h1>Was möchtest du dokumentieren?</h1><p>Starte mit den wichtigsten Daten. Fotos, Dokumente und Historieneinträge kannst du direkt danach ergänzen.</p></div>{error && <p className="form-error">{error}</p>}{upgrade && <div className="upgrade-banner"><span><b>Mehr Platz benötigt?</b> Deine bestehenden Pässe bleiben vollständig erhalten.</span><Link href="/preise">Tarife ansehen →</Link></div>}<form action={createAssetAction} className="form-grid">
      <label className="span-2">Name des Objekts<input name="name" placeholder="z. B. Maschine Halle 2, Firmenfahrzeug oder Klimagerät" maxLength={160} required /></label>
      ...
      <label>Kategorie<select name="category" defaultValue="Sonstiges">{categories.map((category) => <option key={category}>{category}</option>)}</select></label>
      <label>Standort<input name="location" placeholder="z. B. Halle 2 / Lager / Fahrzeug 12" maxLength={200} /></label><label>Hersteller<input name="manufacturer" placeholder="Hersteller" maxLength={160} /></label><label>Modell<input name="model" placeholder="Modell / Typ" maxLength={160} /></label><label>Seriennummer<input name="serialNumber" placeholder="Serien-, Geräte- oder Inventarnummer" maxLength={160} /></label>
      <label>Kauf-/Installationsdatum<input name="purchaseDate" type="date" /></label><label>Garantie bis<input name="warrantyUntil" type="date" /></label><label>Nächster Service / Prüfung<input name="nextServiceDate" type="date" /></label><label>Serviceintervall<select name="serviceIntervalMonths" defaultValue="12"><option value="3">3 Monate</option><option value="6">6 Monate</option><option value="12">12 Monate</option><option value="18">18 Monate</option><option value="24">24 Monate</option><option value="36">36 Monate</option></select></label>
