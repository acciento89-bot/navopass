...
export default async function ServiceEntryPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string; event?: string; job?:string; jobDone?:string }> }) {
  ...
  const job=jobId?(await query<Job>(`SELECT j.id,j.title,j.notes,c.name AS customer_name FROM service_jobs j LEFT JOIN service_customers c ON c.id=j.customer_id WHERE j.id=$1 AND j.user_id=$2 AND j.asset_id=$3 AND j.status='OPEN' LIMIT 1`,[jobId,user.id,asset.id])).rows[0]??null:null;

  if (!roleCanEdit(asset, user.id)) return <main className="app-page"><div className="container"><AppHeader name={user.name} /><div className="page-back"><Link href={`/app/assets/${asset.id}`}>← Zum Objektpass</Link></div><section className="panel"><h1>Nur Lesezugriff</h1><p className="readonly-note">Für einen Service- oder Wartungseintrag brauchst du mindestens die Rolle Bearbeiter.</p></section></div></main>;
