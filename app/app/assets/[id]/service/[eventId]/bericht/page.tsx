...
type Customer = { name:string; contact_name:string|null; street:string|null; postal_code:string|null; city:string|null; country:string|null };

function formatDate(value:string|null){const parsed=dateOnlyAsDate(value);return parsed?new Intl.DateTimeFormat("de-DE",{dateStyle:"long"}).format(parsed):"—";}
...
export default async function ServiceReportPage({params}:{params:Promise<{id:string;eventId:string}>}){
  ...
  if(!asset)notFound();
  const event=(await query<ServiceEvent>(`SELECT id,title,event_type,event_date,description,provider,cost_cents,created_by_name,created_at,labor_minutes,parts_used,measurements,findings,recommendation,customer_name,customer_signature,customer_signed_at FROM asset_events WHERE id=$1 AND asset_id=$2 AND event_type IN ('SERVICE','REPAIR','INSPECTION') LIMIT 1`,[eventId,asset.id])).rows[0];
  if(!event)notFound();
  await ensureCustomerSchema();
  const customer=(await query<Customer>(`SELECT c.name,c.contact_name,c.street,c.postal_code,c.city,c.country FROM assets a LEFT JOIN service_customers c ON c.id=a.service_customer_id WHERE a.id=$1 AND (c.user_id=$2 OR c.id IS NULL) LIMIT 1`,[asset.id,user.id])).rows[0]??null;
  ...
  const detailBlocks=[
    ["Durchgeführte Arbeiten",event.description],
    ["Verbaute Teile / Material",event.parts_used],
    ["Messwerte",event.measurements],
    ["Mängel / Feststellungen",event.findings],
    ["Empfehlung / weitere Maßnahmen",event.recommendation],
  ] as const;

  return <main className={styles.page}><div className={styles.toolbar}><Link className="button ghost" href={`/app/assets/${asset.id}`}>← Zum Objektpass</Link><PrintReportButton /></div>
    <article className={styles.sheet}>
      ...
      <div className={styles.box}><h2>Kunde / Ausführung</h2><dl className={styles.data}><div><dt>Kunde / Standort</dt><dd>{customer?.name||"—"}</dd></div><div><dt>Ansprechpartner</dt><dd>{customer?.contact_name||"—"}</dd></div><div><dt>Adresse</dt><dd>{customerAddress||"—"}</dd></div><div><dt>Techniker / Konto</dt><dd>{event.created_by_name||"—"}</dd></div><div><dt>Ausgeführt durch</dt><dd>{event.provider||"—"}</dd></div><div><dt>Arbeitszeit</dt><dd>{labor(event.labor_minutes)}</dd></div><div><dt>Kosten</dt><dd>{money(event.cost_cents)}</dd></div></dl></div></section>
